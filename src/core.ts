...
export const SUPPORTED_CHAINS: readonly Chain[] = ["base", "ethereum", "bsc"];
...
export function validateAddress(raw: string): string {
  const addr = String(raw ?? "").trim();
  if (!/^0x[0-9a-fA-F]{40}$/.test(addr)) {
    const shown = addr.length > 60 ? addr.slice(0, 60) + "…" : addr;
    throw new InvalidInputError(
      `not a valid EVM address: "${shown}" — expected 0x followed by 40 hex characters`,
    );
  }
  return addr.toLowerCase();
}

export function validateChain(raw: string | undefined): Chain {
...
async function getCode(cfg: ChainConfig, address: string): Promise<CodeResult> {
...
async function queryHoneypotIs(chainId: number, address: string): Promise<HoneypotIsCheck> {
  const url = `https://api.honeypot.is/v2/IsHoneypot?address=${address}&chainID=${chainId}`;
  let status: number, json: any;
  try {
  ...
  const apiFlags = Array.isArray(json?.flags)
    ? json.flags.map((f: any) => (typeof f === "string" ? f : String(f?.flag ?? JSON.stringify(f))))
    : [];
  return {
    reachable: true,
    hasData: true,
...
const GOPLUS_SIGNAL_FIELDS: Array<{ field: string; flag: string; severity: "fail" | "caution" | "info" }> = [
...
interface GoPlusInternal {
  check: GoPlusCheck;
  concentration: TopHolderConcentration | null;
}

async function queryGoPlus(chainId: number, address: string): Promise<GoPlusInternal> {
  ...
  for (const { field } of GOPLUS_SIGNAL_FIELDS) {
    if (data[field] === "1") signals.push(field);
  }
  if (data.is_honeypot === "1") signals.push("is_honeypot");

  let concentration: TopHolderConcentration | null = null;
  if (Array.isArray(data.holders) && data.holders.length > 0) {
    const holders = data.holders.filter(
      (h: any) => h?.address && !BURN_ADDRESSES.has(String(h.address).toLowerCase()),
    );
    const pcts = holders.map((h: any) => fracToPct(h.percent)).filter((p: number | null) => p !== null) as number[];
    concentration = {
...
export async function checkToken(addressRaw: string, chainRaw?: string): Promise<CheckResult> {
  ...
  if (codeR.reachable && typeof codeR.code === "string") {
    ...
    if (code === "0x" || codeBytes === 0) {
      isContract = false;
      explain.push(
        `contract: NO code at ${address} on ${chain} (eth_getCode returned 0x) — this is a wallet (EOA) or an empty address, not a token contract. If you expected a token here, check the chain and re-copy the address.`,
      );
    } else if (code.startsWith("0xef0100") && codeBytes === 23) {
      isContract = false;
      flags.push("eip7702_delegated_eoa");
      explain.push(
        `contract: ${address} is an EOA with an EIP-7702 delegation designator, not a token contract — there is no token to buy at this address.`,
      );
    } else {
      isContract = true;
      explain.push(
        `contract: ${codeBytes} bytes of deployed bytecode at ${address} on ${chain} (chainId ${cfg.chainId}) via eth_getCode — confirmed on-chain contract.`,
      );
    }
  } else {
    flags.push("upstream_unreachable:rpc");
    explain.push(
      `contract: could not reach any ${chain} RPC endpoint (${codeR.error}) — cannot confirm the address holds deployed code.`,
    );
  }
  ...
  if (hp.reachable && hp.hasData) {
    if (hp.isHoneypot) {
      ...
      explain.push(
        `honeypot.is: FLAGGED AS HONEYPOT${hp.honeypotReason ? ` — ${hp.honeypotReason}` : ""}.`,
      );
    } else if (hp.simulationSuccess === false) {
      caution("sell_simulation_failed");
      explain.push(
        `honeypot.is: sell simulation FAILED — could not prove this token can be sold${hp.risk ? ` (risk rating "${hp.risk}")` : ""}. An unproven exit is not the same as a safe one.`,
      );
    } else {
      const taxes = [
      ...
      explain.push(
        `honeypot.is: simulation succeeded — not flagged as honeypot${taxes.length ? ` (${taxes.join(", ")})` : ""}${hp.risk ? `, risk rating "${hp.risk}"` : ""}.`,
      );
    }
  ...
  if (gp.reachable && gp.indexed) {
    ...
    for (const { field, flag, severity } of GOPLUS_SIGNAL_FIELDS) {
      if (gp.signals?.includes(field)) {
        if (severity === "fail") fail(flag);
        else if (severity === "caution") caution(flag);
    ...
    explain.push(
      `goplus: indexed — ${gpBits.join(", ")}${gp.signals && gp.signals.length ? `; raised: ${gp.signals.join(", ")}` : "; no risk fields raised"}.`,
    );
  } else if (gp.reachable) {
    flags.push("goplus_not_indexed");
    explain.push(
      "goplus: token not in the GoPlus index — no security data from this source (common for very new tokens).",
    );
  } else {
  ...
  const sellCandidates = [hp.sellTaxPct, gp.sellTaxPct].filter((v): v is number => typeof v === "number");
  const buyCandidates = [hp.buyTaxPct, gp.buyTaxPct].filter((v): v is number => typeof v === "number");
  ...
  if (gp.sourceVerified === true || hp.openSource === true) sourceVerified = true;
  else if (gp.sourceVerified === false || hp.openSource === false) sourceVerified = false;
  if (isContract === true) {
    if (sourceVerified === false) {
      flags.push("source_not_verified");
      explain.push(
        "source: contract source code is NOT verified on any consulted source — the code doing whatever it does cannot be read.",
      );
  ...
  if (concentration?.topHolderPct !== null && concentration?.topHolderPct !== undefined) {
    if (concentration.topHolderPct > 50) {
      ...
      explain.push(
        `holders: top non-burn holder controls ${concentration.topHolderPct}% of supply — one address can nuke the price.`,
      );
    } else {
      explain.push(
        `holders: top non-burn holder ${concentration.topHolderPct}%, top 10 hold ${concentration.top10Pct}%.`,
      );
    }
  }

  const bothSecuritySourcesEmpty =
    hp.reachable && !hp.hasData && gp.reachable && !gp.indexed;
  ...
  if (isContract === false) {
    ...
    verdict = "fail";
    explain.push(
      "verdict: fail — there is no contract at this address, so there is no token to buy here. Any listing pointing at it is wrong or fake.",
    );
  } else if (failSignals > 0) {
    state = "HONEYPOT_SIGNALS";
    verdict = "fail";
    explain.push(
      `verdict: fail — ${failSignals} honeypot-grade signal(s) raised. Do not buy; you are unlikely to be able to sell.`,
    );
  } else if (degraded) {
    state = "UPSTREAM_DEGRADED";
    verdict = "caution";
    explain.push(
      "verdict: caution — one or more upstream sources were unreachable, so this is a PARTIAL check, not a clean bill of health. Re-run later or verify manually.",
    );
  } else if (sourceVerified === false || bothSecuritySourcesEmpty) {
    state = "UNVERIFIED_RISK";
    verdict = "caution";
    explain.push(
      bothSecuritySourcesEmpty
